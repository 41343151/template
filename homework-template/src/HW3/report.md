...
我認為這次的多項式程式設計作業，讓我了解循環鏈結串列是什麼，來表示抽象資料型別，也實際比較了不同設計選擇（通用類別 vs. 客製化實作、Iterator vs. 指標）所帶來的影響。這讓我理解到，在實務程式設計中，並不存在唯一正確的寫法，而是需要依照問題需求，在可讀性、彈性與效能之間做出取捨。這正是本次作業帶給我最重要的收穫。
